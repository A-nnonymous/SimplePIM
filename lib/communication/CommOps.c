...
*/
void *malloc_scatter_aligned(uint64_t len, uint32_t type_size,
...
void *malloc_reduce_aligned(uint32_t len, uint32_t type_size,
                            simplepim_management_t *table_management) {
  uint32_t num_dpus = table_management->num_dpus;
...
void simplepim_scatter(char *const table_id, void *elements, uint64_t len,
                       uint32_t type_size,
                       simplepim_management_t *table_management) {
  ...
  table_management->free_space_start_pos =
      table_management->free_space_start_pos > t->end
          ? table_management->free_space_start_pos
          : t->end;
}

void *simplepim_gather(char *const table_id,
                       simplepim_management_t *table_management) {
  if (!contains_table(table_id, table_management)) {
    printf(table_id);
  ...
  uint64_t aligned_max_len =
      (max_len * type_size) + (8 - (max_len * type_size) % 8);
  ...
  DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME,
                           start_addr, aligned_max_len, DPU_XFER_DEFAULT));
...
void simplepim_broadcast(char *const table_id, void *elements, uint64_t len,
                         uint32_t type_size,
                         simplepim_management_t *table_management) {
  uint32_t curr_offset = table_management->free_space_start_pos;
  if (contains_table(table_id, table_management)) {
  ...
  add_table(t, table_management);
  table_management->free_space_start_pos =
      t->end > table_management->free_space_start_pos
          ? t->end
          : table_management->free_space_start_pos;
}

void simplepim_allgather(char *const table_id, char *const new_table_id,
                         simplepim_management_t *table_management) {
  if (!contains_table(table_id, table_management)) {
    printf(table_id);
  ...
  void *bc_buffer =
      malloc_broadcast_aligned(total_len, type_size, table_management);
  void *res = simplepim_gather(table_id, table_management);
  memcpy(bc_buffer, res, total_len * type_size);
  simplepim_broadcast(new_table_id, bc_buffer, total_len, type_size,
                      table_management);

  free(bc_buffer);
}

void simplepim_allreduce(char *const table_id, handle_t *binary_handle,
                         simplepim_management_t *table_management) {
  if (binary_handle->func_type == 1) {
    ...
    void *bc_buffer =
        malloc_broadcast_aligned(len, type_size, table_management);
    ...
    uint64_t broadcast_size = (len * type_size) + 8 - (len * type_size) % 8;
    struct dpu_set_t set = table_management->set;
    DPU_ASSERT(dpu_broadcast_to(set, DPU_MRAM_HEAP_POINTER_NAME, outputs_pos,
                                bc_buffer, broadcast_size, DPU_XFER_DEFAULT));
