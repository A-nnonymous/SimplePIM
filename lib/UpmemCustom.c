...
uint32_t calculate_pad_len(uint64_t len, uint32_t type_size,
                           uint32_t num_dpus) {
...
void *malloc_split_aligned(uint32_t len, uint32_t type_size,
                           uint32_t num_dpus) {
...
void prepare_input_len_and_parse_args(struct dpu_set_t set,
                                      dpu_arguments_t *input_args,
                                      uint32_t input_len,
                                      uint32_t input_type_size,
                                      uint32_t num_dpus) {
  uint32_t pad_len_in_elem =
      calculate_pad_len(input_len, input_type_size, num_dpus) / input_type_size;
  uint32_t num_transfered_elem_per_dpu =
      (input_len + pad_len_in_elem) / num_dpus;
  ...
  // DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_TO_DPU, "DPU_INPUT_ARGUMENTS", 0,
...
uint32_t host_split_to_dpu(struct dpu_set_t set, void *elements, uint32_t len,
                           uint32_t type_size, uint32_t num_dpus,
                           uint32_t curr_offset) {
  // elements must be 8 bytes aligned and divisible by num_dpus， pad not
  // aglined, use malloc_aligned
  uint32_t pad_len = calculate_pad_len(len, type_size, num_dpus);

  // split elements to dpu and rest remains on host
  uint32_t len_per_dpu_in_byte =
      ((uint64_t)len * type_size + pad_len) / num_dpus;
...
uint32_t host_broadcast_to_dpu(struct dpu_set_t set, void *elements,
                               uint32_t len, uint32_t type_size,
                               uint32_t curr_offset) {
  uint32_t broadcast_size = (len * type_size) + (len * type_size) % 8;
  DPU_ASSERT(dpu_broadcast_to(set, DPU_MRAM_HEAP_POINTER_NAME, curr_offset,
                              elements, broadcast_size, DPU_XFER_DEFAULT));
  return curr_offset + broadcast_size;
}

void *malloc_gather_aligned(uint32_t len, uint32_t type_size,
                            uint32_t num_dpus) {
...
void gather_tables_to_host(struct dpu_set_t set, void *my_table, uint32_t len,
                           uint32_t type_size, uint32_t curr_offset_on_mram,
                           uint32_t num_dpus,
  ...
  DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME,
                           curr_offset_on_mram, aligned_table_size,
                           DPU_XFER_DEFAULT));
  ...
  omp_set_dynamic(
      0); // Explicitly disable dynamic teams private(omp_table, thread_id)
...
void *gather_to_host(struct dpu_set_t set, uint32_t *lens, uint32_t type_size,
                     uint32_t curr_offset_on_mram, uint32_t num_dpus) {
  ...
  DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME,
                           curr_offset_on_mram, aligned_max_len,
                           DPU_XFER_DEFAULT));
