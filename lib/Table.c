...
void init_table(table_t *t, uint32_t table_size, uint32_t value_size,
  ...
  t->table_allocator =
      fsb_alloc(table_size * value_size + (table_size * value_size) % 8, 1);
...
void insert_table(table_t *t, uint32_t key, void *value,
...
void free_table(table_t *t) { fsb_free(t->table_allocator, t->table); }
...
void combine_tables(table_t *t1, table_t *t2,
                    void (*combineFunc)(void *, void *)) {
...
void combine_tables_lockfree(__mram_ptr void *table_entries_mram,
                             table_t *local_table,
                             void (*init_func)(uint32_t, void *),
                             void (*combineFunc)(void *, void *)) {
  ...
  if (num_tasklets == 1) {
    ...
    for (int i = 0; i < num_tasklets; i++) {
      ...
      start_pos = curr_id < rest ? (local_len + 1) * curr_id
                                 : local_len * curr_id + rest;
      combine_table_entries(global_table_ptr + start_pos * value_size,
                            local_table_ptr + start_pos * value_size, curr_len,
                            value_size, combineFunc);

      barrier_wait(&barrier);
    }

    uint32_t transfer_size =
        table_size * value_size + (table_size * value_size) % 8;
    store_arr_aligned(global_table_helper->table, table_entries_mram,
                      transfer_size);
