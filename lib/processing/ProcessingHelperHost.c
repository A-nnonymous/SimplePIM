...
handle_t *create_handle(const char *func_pathname, uint32_t func_type) {
  ...
  if (func_type == 0) {
    char compile_cmd[2048] = "dpu-upmem-dpurte-clang -O2 -DNR_TASKLETS=12 -I./ "
                             "-D__mapfunc_filename__=\"<";
    strcat(compile_cmd, func_pathname);
    strcat(compile_cmd, "/map.h>\" -o bin/dpu_map_");
    strcat(compile_cmd, func_bodyname);
    strcat(compile_cmd, " ../../lib/processing/map/map_dpu.c "
    ...
    strcat(compile_cmd,
           ...
           "../../lib/processing/ProcessingHelper.c ../../lib/TableHost.c "
           "../../lib/Table.c ../../lib/TableShared.c");
    int succ = system(compile_cmd);
    ...
    char compile_cmd[2048] =
        "dpu-upmem-dpurte-clang -O2 -DNR_TASKLETS=12 -o bin/dpu_zip";
    strcat(compile_cmd, " ../../lib/processing/zip/zip_dpu.c "
