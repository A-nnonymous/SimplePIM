...
int main() {
  ...
  if (is_zipped) {
    zip_map_dpu(DPU_MRAM_HEAP_POINTER + start1, DPU_MRAM_HEAP_POINTER + start2,
                DPU_MRAM_HEAP_POINTER + output_start_offset, type1, type2,
                output_type_size, len);
  } else {
    map_dpu(DPU_MRAM_HEAP_POINTER + input_start_offset,
            DPU_MRAM_HEAP_POINTER + output_start_offset, input_type_size,
            output_type_size, len);
  }
  return 0;
}