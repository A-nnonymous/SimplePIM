...
void average_table_entries_to_arr(void *centroid_table, void *centroids) {
  ...
  for (int i = 0; i < k; i++) {
    ...
    src_ptr = centroid_table + i * (dim * sizeof(T) + sizeof(int32_t)) +
              sizeof(uint32_t);
...
void run() {
  ...
  // inputs
  T *elements = (T *)malloc_scatter_aligned(num_elements, dim * sizeof(T),
                                            table_management);
  ...
  for (int m = 0; m < iter; m++) {
    ...
    T *res = table_gen_red("t1", "t3", dim * sizeof(T) + sizeof(int32_t), k,
