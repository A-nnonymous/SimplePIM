...
// table_host_t holds information about one array, used by the framework and the
// end user does not need to care about its details
typedef struct {
  char *name;
  uint32_t start;
  uint32_t end;
...
// simplepim_management_t holds information about all registered arrays, the pim
// hardware like number of pim cores etc. (see the paper for detials)
typedef struct {
  uint32_t curr_space;
  uint32_t num_tables;
  table_host_t **tables;
  ...
  // one could do memory management with more complex logic, currently just
  // increment counter for each new array
  uint32_t free_space_start_pos;
} simplepim_management_t;

/*
   table_management_init initialise a management interface, can be called by end
   users add_table, contains_table, lookup_table, and free_table are used to
   retrieve information of an array from the management unit with its array id
   "name" (see the paper for detials) max_len_dpu is a helper function used by
   the framework
*/
...
table_host_t *lookup_table(const char *name,
