...
void gen_red_dpu(__mram_ptr void *inputs, __mram_ptr void *outputs,
  ...
  uint32_t divisible_len_in_bytes =
      ((len >> copy_block_size_shiftbits) << copy_block_size_shiftbits) *
      elem_type_size;
  ...
  if (shift_bits == 0) {
    for (int i = pid * copy_block_size_in_bytes; i < divisible_len_in_bytes;
      ...
      for (uint32_t j = (uint32_t)elems_block; j < copy_block_end;
           j += elem_type_size) {
  ...
  if (pid == NR_TASKLETS - 1 && rest_len_in_bytes != 0) {
    ...
    for (uint32_t j = (uint32_t)elems_block;
         j < (uint32_t)elems_block + rest_len_in_bytes; j += elem_type_size) {
      // intermediate results
      map_to_val_func((void *)j, tmp_intermediate, &key);
      // get entry for intermediate results
      curr_entry = key * inter_type_size;
