...
void gather_tables_to_host(simplepim_management_t *table_management,
                           void *my_table, uint32_t len, uint32_t type_size,
                           uint32_t curr_offset_on_mram,
...
void *table_gen_red(const char *src_name, const char *dest_name,
  ...
  if (binary_handle->func_type == 1) {
    ...
    DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_TO_DPU, "GEN_RED_INPUT_ARGUMENTS", 0,
                             sizeof(gen_red_arguments_t), DPU_XFER_DEFAULT));

    gettimeofday(&end_time, NULL);
    double prepare_args_time =
    ...
    gather_tables_to_host(table_management, my_table, output_len, output_type,
                          outputs, init_func, combine_func);
    dlclose(lib);
    gettimeofday(&end_time, NULL);
    double host_table_reduction_time =
        (end_time.tv_sec - start_time.tv_sec) * 1000000.0 +
        (end_time.tv_usec - start_time.tv_usec);
    // back to dpus
