...
uint64_t _copy_block_size(uint32_t type_size1, uint32_t type_size2,
                          uint32_t num_elem) {
  // 1024
  uint64_t res = 0;
  uint32_t res_arr[2];
  uint32_t max_type_size = type_size1 > type_size2 ? type_size1 : type_size2;
  if (type_size1 % 8 == 0 && type_size2 % 8 == 0 &&
      (num_elem <= NR_TASKLETS || max_type_size > 512)) {
    res_arr[0] = 1;
    res_arr[1] = 0;
  } else if (type_size1 % 4 == 0 && type_size2 % 4 == 0 &&
             (num_elem <= 2 * NR_TASKLETS || max_type_size > 256)) {
    res_arr[0] = 2;
    res_arr[1] = 1;
  } else if (type_size1 % 2 == 0 && type_size2 % 2 == 0 &&
             max_type_size > 128) {
    res_arr[0] = 4;
    res_arr[1] = 2;
...
void map_dpu(__mram_ptr void *inputs, __mram_ptr void *outputs,
  ...
  fsb_allocator_t elems_block_allocator =
      fsb_alloc(elem_type_size << copy_block_size_shiftbits, 1);
  __dma_aligned void *elems_block = fsb_get(elems_block_allocator);

  fsb_allocator_t inter_block_allocator =
      fsb_alloc(inter_type_size << copy_block_size_shiftbits, 1);
  ...
  uint32_t divisible_len = (len >> copy_block_size_shiftbits)
  ...
  uint32_t j_elem_unroll_block_end =
      (uint32_t)(unroll_block_size * elem_type_size + elems_block);
  uint32_t j_inter_unroll_block_end =
      (uint32_t)(unroll_block_size * inter_type_size + inter_block);
  ...
  for (uint32_t i_elem = i_elem_init; i_elem < i_elem_divisible_len;
    ...
    for (uint32_t j_elem = j_elem_unroll_block_end; j_elem < j_elem_block_end;
         j_elem += elem_type_size) {
  ...
  if (pid == 0 && rest_len != 0) {
    ...
    mram_read((__mram_ptr void *)(outputs + last_block_inter_addr), inter_block,
              block_size_outputs);
    mram_read((__mram_ptr void *)(inputs + last_block_elem_addr), elems_block,
              block_size_inputs);
    ...
    mram_write(inter_block,
               (__mram_ptr void *)(outputs + last_block_inter_addr),
               block_size_outputs);
  }
...
void zip_dpu(__mram_ptr void *table_entries_1, __mram_ptr void *table_entries_2,
             ...
             uint32_t input_type_2, uint32_t len) {
  uint32_t num_tasklets = NR_TASKLETS;
  uint32_t pid = num_tasklets == 1 ? 0 : me();
  ...
  if (pid == 0 && rest_len != 0) {
    ...
    mram_read((__mram_ptr void *)table_entries_1 + last_block_input1_addr,
              input1_block, input_block_size_1);
    mram_read((__mram_ptr void *)table_entries_2 + last_block_input2_addr,
              input2_block, input_block_size_2);
    mram_read((__mram_ptr void *)table_entries_res + last_block_output_addr,
    ...
    mram_write(output_block, table_entries_res + last_block_output_addr,
               output_block_size);
  }
...
void map_and_combine_oncache_imbalanced(
    __mram_ptr void *table_entries_mram, void (*init_func)(uint32_t, void *),
    void (*mapHashFunc)(void *, void *, uint32_t *),
    void (*combineFunc)(void *, void *), dpu_arguments_t *DPU_INPUT_ARGUMENTS) {
  ...
  __mram_ptr void *elements =
      DPU_MRAM_HEAP_POINTER + DPU_INPUT_ARGUMENTS->input_start_offset;
  // try malloc/free for performance
  fsb_allocator_t elems_block_allocator =
      fsb_alloc(copy_block_size * elem_type_size, 1);
  __dma_aligned void *elems_block = fsb_get(elems_block_allocator);
  ...
  uint32_t key = 0;
  // TODO : possible optimisation : shifts instead of mult, check bounds for
  // last iterations (later)

  curr_block = 0;
  uint32_t i;
  uint32_t curr_block_local;
  uint32_t num_blocks = (len % copy_block_size == 0)
                            ? len / copy_block_size
                            : len / copy_block_size + 1;
  ...
  combine_tables_lockfree(table_entries_mram, local_table, init_func,
                          combineFunc);
...
void combine_oncache_dpu(__mram_ptr void *table_entries_mram,
                         dpu_arguments_t *DPU_INPUT_ARGUMENTS) {
  uint32_t len = DPU_INPUT_ARGUMENTS->input_len;
  uint32_t elem_type_size = DPU_INPUT_ARGUMENTS->input_type_size;
  uint32_t inter_type_size = DPU_INPUT_ARGUMENTS->table_type_size;
  uint32_t table_size = DPU_INPUT_ARGUMENTS->table_len;
...
void combine_shared_dpu(__mram_ptr void *table_entries_mram,
