...
void table_zip(const char *src1_name, const char *src2_name,
  ...
  if (binary_handle->func_type == 2) {
    ...
    if (src1_table->is_virtual_zipped == 1) {
      ...
      DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_TO_DPU, "ZIP_INPUT_ARGUMENTS", 0,
                               sizeof(zip_arguments_t), DPU_XFER_DEFAULT));
      gettimeofday(&start_time, NULL);
    ...
    uint32_t max_end_dpu =
        max_len_dpu(num_dpus, src1_table) * output_type + outputs;
