...
void table_map(const char *src_name, const char *dest_name,
  ...
  if (binary_handle->func_type == 0 && src_table->is_virtual_zipped == 0) {
    ...
    DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_TO_DPU, "MAP_INPUT_ARGUMENTS", 0,
                             sizeof(map_arguments_t), DPU_XFER_DEFAULT));
    ...
    t->start = outputs;
    uint32_t max_end_dpu =
        max_len_dpu(num_dpus, src_table) * output_type + outputs;
    ...
    printf("--------------\n");
  } else if (binary_handle->func_type == 0 &&
             src_table->is_virtual_zipped == 1) {
    // timing
    gettimeofday(&start_time, NULL);
