...
// multithreaded load and store, len must be 8 bytes aligned, can operate on
// more than 2kB data
...
void load_arr_aligned(void *arr, __mram_ptr void *heap_ptr, uint32_t len) {
  ...
  uint32_t last_transfer_size =
      (transfer_size % transfer_each_time == 0)
          ? transfer_each_time
          : transfer_size / transfer_size % transfer_each_time;
  uint32_t num_blocks = (transfer_size % transfer_each_time == 0)
                            ? transfer_size / transfer_each_time
                            : transfer_size / transfer_each_time + 1;
