...
void zip_map_dpu(__mram_ptr void *inputs1, __mram_ptr void *inputs2,
                 __mram_ptr void *outputs, uint32_t input_type1,
                 uint32_t input_type2, uint32_t output_type, uint32_t len) {
  ...
  fsb_allocator_t elems_block_allocator_tmp =
      fsb_alloc(elem_type_size << copy_block_size_shiftbits, 1);
  ...
  if (pid == NR_TASKLETS - 1 && rest_len != 0) {
    ...
    mram_read((__mram_ptr void *)(outputs + last_block_inter_addr), inter_block,
              block_size_outputs);
    mram_read((__mram_ptr void *)(inputs1 + last_block_elem1_addr),
              inputs1_block, input1_block_size);
    mram_read((__mram_ptr void *)(inputs2 + last_block_elem2_addr),
              inputs2_block, input2_block_size);
